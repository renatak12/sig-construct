...
class RealizarVendaViewTest(TestCase):
    def setUp(self):
        ...
        self.user = Usuario.objects.create_user(username='testuser', password='testpassword')
        self.client.login(username='testuser', password='testpassword')
        self.realizar_venda_url = reverse('stock:realizar_venda')
        ...
        self.produto = Produto.objects.create(
            nome='Produto Teste', preco=10.0, quantidade_estoque=100, categoria=self.categoria
        )

    def test_realizar_venda_get(self):
        response = self.client.get(self.realizar_venda_url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'stock/vendas/realizar.html')

    def test_realizar_venda_post_success(self):
        # Crie uma categoria para o produto
        categoria = Categoria.objects.create(nome='Categoria Teste')
        
        # Crie um produto com categoria associada
        produto = Produto.objects.create(
            nome='Produto Teste',
            preco=10.0,
            quantidade_estoque=100,
            categoria=categoria,  # Certifique-se de associar uma categoria ao produto
        )
        
        cliente = Cliente.objects.create(nome='Cliente Teste', telefone='1234567890')

        # Realize a venda de 10 unidades do produto
        self.client.post(self.realizar_venda_url, {
            'cliente': cliente.id,
            'produto': produto.id,
            'quantidade': 10,
            'parcelamento': 1,
        ...
        # Verifique se a quantidade de venda foi corretamente registrada na venda
        venda = Venda.objects.last()
        self.assertEqual(venda.quantidade, 10)
